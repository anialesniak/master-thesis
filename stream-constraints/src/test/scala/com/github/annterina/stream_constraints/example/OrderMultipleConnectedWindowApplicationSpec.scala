...
class OrderMultipleConnectedWindowApplicationSpec extends AnyFunSpec with BeforeAndAfterEach {
  ...
  describe("Order Application With Multiple Connected Window Constraints") {
    ...
    it("should swap events in the window with single after event") {
      ...
      assert(fifthOutput.key == "111")
      assert(fifthOutput.value.key == 1)
      assert(fifthOutput.value.action == "CREATED")
    }
  }
}
